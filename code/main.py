...
from preprocessing import load_mnist, load_wikiart, convert_to_tensor_dataset_1, convert_to_tensor_dataset_2
...
def train_vae(model, train_loader, args, is_cvae=False):
    ...
    total_loss = 0
    batched_dataset = train_loader.batch(args.batch_size)
    for batch in batched_dataset:
        ...
        with tf.GradientTape() as tape:
            if(is_cvae):
                x_hat, mu, log_var = model.call(input, one_hot(label, 10))
            else:
                x_hat, mu, log_var = model.call(input)
            loss = loss_function(x_hat, input, mu, log_var)
            total_loss += loss
...
def main(args):
    ...
    convert_to_tensor_dataset_2(data, label_index, 256)
